...
/*  postfix.pl  --  Primary expressions with trailing access / call chains,
    ...
    Produced AST (each node, and each accessor `label`/`index`, additionally
    carries a trailing `span(Start, End)` of source offsets -- see
    `parser/position.pl`):
...
:- use_module(position, [here//1, span_between/3, node_span/2, span_cover/3, offset/2]).
...
% Each built node spans from the start of its target to the end of the operation
% just consumed (`here(End)`); `node_span/2` reads the target's recorded start.
postfix_chain(ExpressionFunctor, Target, Node) -->
  ".",
  accessor(Accessor),
  here(EndTail),
  { node_span(Target, span(Start, _)),
    offset(EndTail, End),
    Accessed = access_node(Target, Accessor, span(Start, End))
  },
  postfix_chain(ExpressionFunctor, Accessed, Node).
postfix_chain(ExpressionFunctor, Target, Node) -->
  separators,
  "(",
  separators,
  ...
  { node_span(Target, span(Start, _)),
    offset(EndTail, End),
    Called = function_call_node(Target, Arguments, span(Start, End))
  },
  postfix_chain(ExpressionFunctor, Called, Node).
postfix_chain(_, Target, Target) --> [].

accessor(label(Name, Span)) -->
  here(Start),
  identifier(identifier_node(Name, _)),
  here(End),
  { span_between(Start, End, Span) }.
accessor(index(Index, Span)) -->
  here(Start),
  decimal_digit(First),
  decimal_digits(Rest),
  here(End),
...
call_argument(_, placeholder_node(Span)) -->
...
% start of `==`.  The assignment node spans from the access target to the value.
optional_assignment(ExpressionFunctor, AccessNode,
                    assignment_node(AccessNode, Value, Span)) -->
  { AccessNode = access_node(_, _, _) },
  separators,
  assignment_operator,
  separators,
  phrase(ExpressionFunctor, Value),
  { span_cover(AccessNode, Value, Span) }.
